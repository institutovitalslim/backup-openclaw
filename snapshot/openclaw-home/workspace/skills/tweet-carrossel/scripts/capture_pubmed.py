#!/usr/bin/env python3
"""
capture_pubmed.py — Captura screenshot da pagina PubMed com MULTIPLAS estrategias.

NUNCA FALHA: sempre retorna uma imagem valida, usando fallbacks em cascata:
1. Tenta PubMed direto com 6 user-agents diferentes
2. Tenta via cache archive.org
3. Tenta PMC (PubMed Central) se disponivel
4. Ultimo recurso: gera imagem sintetica com metadados do paper via eutils
"""
import argparse, subprocess, os, sys, time, json, urllib.request
...
def is_valid_screenshot(png_path, min_size=50_000):
    """Valida que o screenshot é da pagina PubMed real, NAO captcha/erro."""
    ...
    try:
        ...
        # Check 1: header NIH azul (cor especifica 32,84,147)
        # A barra NIH fica no topo, bem larga
        nih_strip = img.crop((0, 0, img.width, 80))
        pixels = list(nih_strip.getdata())
        # Conta pixels proximos do azul NIH (tolerancia larga)
        ...
        # Check 2: nao eh pagina predominantemente branca (captcha tipico)
...
def try_chromium(url, out_path, ua, wait_ms=8000, attempt_id=0):
    """Chromium com flags anti-bot-detection e perfil limpo."""
    import tempfile, shutil
    profile_dir = tempfile.mkdtemp(prefix=f"chromium_{attempt_id}_")
    try:
        subprocess.run([
...
def fetch_paper_metadata(pmid):
    """Busca metadados via NCBI eutils API (mais confiavel que scraping)."""
    try:
        ...
        print(f"    eutils falhou: {e}")
        return None


def render_synthetic(pmid, meta, out_path):
    """Gera imagem sintetica estilo PubMed com os metadados reais do paper."""
    print("  Estrategia 4: gerando imagem sintetica com metadados eutils...")
    W, H = 1200, 900
...
def capture(pmid, out_path):
    ...
    # Estrategia 1: tentar todos os user-agents direto
    print("Estrategia 1: PubMed direto com multiplos user-agents + perfil limpo")
    ...
    # Estrategia 2: archive.org cache
    print("Estrategia 2: Archive.org Wayback Machine")
    archive_url = f"https://web.archive.org/web/2024/{url_direct}"
    for ua in USER_AGENTS[:3]:
        if try_chromium(archive_url, out_path, ua, wait_ms=12000):
            valid, msg = is_valid_screenshot(out_path)
            print(f"    {msg}")
            if valid:
                return True
    ...
    # Estrategia 3: buscar metadados via eutils e tentar PMC
    print("Estrategia 3: eutils + PMC (se disponivel)")
    meta = fetch_paper_metadata(pmid)
    if meta and meta.get("pmcid"):
        pmc_url = f"https://www.ncbi.nlm.nih.gov/pmc/articles/{meta['pmcid']}/"
        for ua in USER_AGENTS[:3]:
            if try_chromium(pmc_url, out_path, ua):
                valid, msg = is_valid_screenshot(out_path)
                print(f"    PMC: {msg}")
                if valid:
                    return True

    # Estrategia 4: gerar imagem sintetica com metadados reais
    if meta:
        render_synthetic(pmid, meta, out_path)
        valid, msg = is_valid_screenshot(out_path, min_size=10_000)
        print(f"    Sintetico: {msg}")
        if valid:
            return True

    # Estrategia 5 (ultima): imagem placeholder
    print("Estrategia 5: placeholder")
    ...
    img.save(out_path)
    return True  # sempre retorna True — nunca trava o pipeline
...
def main():
    ...
    if capture(args.pmid, args.out):
        ...
        print(f"FALHA INESPERADA", file=sys.stderr)
        sys.exit(1)
