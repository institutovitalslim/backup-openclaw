...
IGNORE_PARTS = {'site-packages','node_modules','.git','venv','.venv','__pycache__','snapshot','memory','docs','dist','build'}
...
FORBIDDEN = {'anthropic/claude-sonnet-4-6'}
...
for root in ROOTS:
    ...
    for p in root.rglob('*'):
        ...
        if p.suffix and p.suffix not in ALLOWED_EXTS: continue
        if not any(h in str(p).lower() for h in OPERATIONAL_HINTS): continue
        try:
        ...
        for i,line in enumerate(lines,1):
            for pat in PATS:
                ...
                if model in FORBIDDEN: sev='forbidden'
                findings.append({'path':str(p),'line':i,'model':model,'severity':sev,'text':line.strip()})
                break

summary={
    'total': len(findings),
    'forbidden': sum(1 for x in findings if x['severity']=='forbidden'),
...
report={
  ...
  'forbidden_models': sorted(FORBIDDEN),
  'findings': findings,
}
print(json.dumps(report, ensure_ascii=False, indent=2))
if summary['forbidden'] > 0:
    print('\nALERT: forbidden LLM override found in operational files.', file=sys.stderr)
    sys.exit(2)
