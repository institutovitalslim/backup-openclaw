...
- Antes de sugerir algo em `content`, `tasks`, `recommendations` ou `digest`, consulte nesta ordem: `decisions`, `lessons`, `feedback`.
...
- Se houver conflito entre memorias, priorize `decisions`, depois `lessons`, depois `feedback` recente.

## Semantic Memory

- Na inicializacao da sessao, carregue apenas `SOUL.md`, `USER.md`, `IDENTITY.md` e a nota diaria `memory/YYYY-MM-DD.md`.
...
- Trate notas diarias como memoria bruta; trate `MEMORY.md`, `lessons`, `decisions`, `topic-*` e `feedback_loops` como memoria curada.
