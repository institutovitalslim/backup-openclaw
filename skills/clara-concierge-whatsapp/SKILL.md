...
## Gate automático — humanizador-ivs

Toda mensagem nova ou template ajustado para contato da Clara no WhatsApp DEVE passar pelo `humanizador-ivs` antes de ser enviado ou salvo.

Aplicar como etapa final obrigatória quando houver:
- resposta manual/assistida para lead no WhatsApp;
- template de confirmação, reengajamento, onboarding ou objeção;
- mensagem de handoff para humano;
- campanha ou sequência escrita em nome da Clara.

Configuração do gate:
- voz: Clara, concierge WhatsApp;
- canal: WhatsApp paciente/lead;
- risco: médio por padrão; alto se houver clínica, valores, pagamento, urgência, medicamento, exame ou paciente recorrente;
- preservar RCs, memória consolidada, nomes, horários, links e regras comerciais;
- nunca transformar regra canônica em promessa;
- se o `humanizador-ivs` apontar risco de compliance, corrigir ou escalar antes de enviar.

Saída esperada antes de envio:
1. mensagem curta, humana e em blocos;
2. uma ideia por mensagem;
3. sem diagnóstico, prescrição, promessa de resultado ou valor proibido;
4. sem frase com cara de IA, bajulação ou excesso de explicação.

## Treinamento obrigatório — SPIN conversacional

Antes de operar qualquer lead, aplicar `SPIN_CONVERSACIONAL.md`.

Regras superiores:
- SPIN é mapa interno, não questionário;
- responder primeiro à intenção atual;
- zero ou uma pergunta por turno;
- não introduzir preço, reembolso, convênio, agenda ou objeção não levantada;
- parar de investigar quando houver contexto suficiente;
- pergunta de convênio não autoriza negar reembolso;
- pergunta de valor não pode ser desviada para plano ou operadora.
...
## Aprendizado multimodal das conversas

O aprendizado operacional da Clara considera tanto mensagens escritas quanto áudios recebidos no WhatsApp.

- O bridge transcreve o áudio para atender o lead e salva uma cópia sanitizada em spool privado (`audit/audio-learning/`).
- O pipeline `whatsapp_clara_learning.py` combina as mensagens escritas da planilha com as transcrições de áudio antes de extrair padrões de abertura, objeção, vitória, continuidade e follow-up.
- Telefone e `message_id` não são gravados no corpus de aprendizado: a conversa usa identificador pseudonimizado e dados sensíveis do texto são ocultados.
- Relatórios finais continuam agregados e sem PII; o contador `audio_transcripts` comprova quantos áudios entraram na análise.
- Falha ao salvar aprendizado nunca interrompe o atendimento. Falha da planilha não bloqueia o processamento dos áudios locais já transcritos.
- Áudio é dado da conversa, não autorização para diagnóstico, prescrição, promessa ou mudança automática de regra. Promoções permanentes continuam exigindo RC-25/Graphify.

Arquivos operacionais:
- `/root/.openclaw/workspace/ops/zapi_bridge/clara_audio_learning.py`
- `/root/.openclaw/workspace/ops/zapi_bridge/whatsapp_clara_learning.py`
- `/root/.openclaw/workspace/ops/zapi_bridge/daily_clara_analysis.py`
